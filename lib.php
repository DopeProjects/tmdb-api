...
class Tmdb_model {
  ...
  function __construct()
    function get_movie_info($tmdb_id='')
    {
      ...
      $data           = file_get_contents('https://api.themoviedb.org/3/movie/'.$tmdb_id.'?api_key=xxxxxxxxxxxxxxxxxxx');
  ...
  function get_tvseries_info($tmdb_id='')
    {
      ...
      $data           = file_get_contents('https://api.themoviedb.org/3/tv/'.$tmdb_id.'?api_key=xxxxxxxxxxxxxxxxxxx');
      $data           = json_decode($data, true);
      if(isset($data['error_message'])){
        $response['status']    = 'fail';
      }else{
  ...
  function get_tvshow_actor_info($tmdb_id='')
    {
      ...
      $data           = file_get_contents('https://api.themoviedb.org/3/tv/'.$tmdb_id.'/credits?api_key=xxxxxxxxxxxxxxxxxxx');
      $data           = json_decode($data, true);
      if(isset($data['error_message'])){
        $response['status']    = 'fail';
      }else{
        // var_dump($data);
        // echo "<pre></pre>";
        // var_dump($data['credits']);
        $actors         = array();
        $directors      = array();
        $writters       = array();
        if(count($data) >0){
          $actors       = $this->update_actors($data['cast']);
          $stars        = $this->update_directors_writers($data['crew']);
          $added_star   = $actors + $stars;
        }
      }
      return $added_star;
  ...
  function search($title='',$to=''){
      ...
      if($to =='tv'):
        $data           = file_get_contents('https://api.themoviedb.org/3/tv/'.$title.'?api_key=xxxxxxxxxxxxxxxxxxx');
      else:
        $data           = file_get_contents('https://api.themoviedb.org/3/movie'.rawurlencode($title).'?api_key=xxxxxxxxxxxxxxxxxxx');
      endif;
      $data           = json_decode($data, true);
